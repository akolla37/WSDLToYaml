...
* @param location  This can be url or content or MEMORY
* @param fullPath This is the path to the file if on disk
* @param auth is the auth credentials if if location is a url...it is the content if location is MEMORY
...
*/
function asContent(location, fullPath, auth, context, req) {
    var def = q.defer();
...
*/
function asArchive(content, req, fileName, isLegacyStyle) {
    ...
    try {
        if (isArchive(content)) {
            ...
            loadFiles(legacyArchive, req).then(function(archive) {
                def.resolve(archive);
            }, function(err) {
                def.reject(err);
            });
...
function loadFiles(legacyArchive, req, files) {
    if (!files) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        let dom = d.loadDOM(decodedContent);
        let locations = d.getLocations(dom, fileName, req);
        if (locations.length > 0) {
            allLocations.push(locations);
        }
    }

    // Determine which locations are locations taht have not been loaded
    let newLocations = _.pullAll(_.uniq(_.flatten(allLocations)), knownFiles);
    if (newLocations.length === 0) {
        def.resolve(legacyArchive);
    } else {

        // Load the new locations
        let promiseList = [];
        for (let i = 0; i < newLocations.length; i++) {
            promiseList.push(asContent(newLocations[i], newLocations[i], null, null, req));
        }
        ...
        Promise.all(promiseList).then(values => {
            ...
            for (let i = 0; i < values.length; i++) {
                let content = encode(values[i].content, values[i].encoding);
                let loaded = {
            ...
            loadFiles(legacyArchive, req, files).then(function() {
                def.resolve(legacyArchive);
            }, function(err) {
                def.reject(err);
            });
...
function wsiEncodingCheck(encoding, fileName, req) {
    if (encoding) {
        ...
        if (encoding === 'utf8' || 'utf16' || 'utf-8' || 'utf-16') {
            // Okay
        } else {
...
*/
function pipeArchiveZip(inputArchive, req, ignore, process) {
    ...
    try {
        if (isZip(input)) {
                ...
                if (err) {
                    ...
                    if (err.message.indexOf('relative path') >= 0) {
                        ...
                        let detail = g.http(u.r(req)).f('The content may have been uploaded with a wrong encoding (i.e. utf8). ' +
                          'Here are the first 50 base64 characters of the content [%s].', input.toString('base64', 0, 50));
            ...
            fileDef.promise.then(function(data) {
                outZip.generateAsync({
                    type: 'nodebuffer',
                    compression: 'DEFLATE'
                }).then(function(content) {
...
function defaultIgnore(fileName, mode, req) {
    ...
    if (isXSD(fileName) || isWSDL(fileName) || isXML(fileName) || isConfig(fileName)) {
        return false;
    } else {
        return true;
    }
