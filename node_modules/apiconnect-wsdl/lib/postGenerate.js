...
function duplicatePolyHierarchy(swagger, dict) {
    ...
    try {
        ...
        R.error(req, g.http(u.r(req)).f('An unexpected error (%s) was encountered while constructing the \'subTypes map\'. Processing continues.', e));
    }
        ...
        for (nsName in swagger.definitions) {
            ...
            if (map[nsName]) {
                ...
                for (var subType in map[nsName]) {
                    ...
                    if (!swagger.definitions[newNSName] &&
                ...
                // Clone the ancestor hiearchy of the nsName
                ...
                if (anc) {
                    for (let i = 0; i < anc.length; i++) {
                        ...
                        if (!swagger.definitions[newAncNSName] &&
                            ...
                            count++;
                            // If there is an allOf, change it to the new base
                            if (swagger.definitions[newAncNSName]['allOf'] && swagger.definitions[newAncNSName]['allOf'][0]['$ref']) {
        ...
        for (var nsName in definitions) {
            ...
            if (dictEntry &&
                ...
                if (!dictEntry.preventOptimize &&
                    ...
                    definitions[nsName].xml &&
                    definitions[nsName].xml.namespace == definitions[linkup].xml.namespace) {
...
function replaceRefs(swagger, map) {
    return u.traverse(swagger, function(obj) {
        if (obj && obj.$ref) {
            ...
            if (map[nsName]) {
                obj.$ref = '#/definitions/' + map[nsName];
            }
        }
        return obj;
    });
...
function inlineSwaggerAttributes(swagger) {
    ...
    if (Object.keys(attrMap).length > 0) {
        u.traverseXSO(swagger, function(xso) {
            if (xso.$ref) {
...
function findRefsAndNullables(swagger) {
    ...
    u.traverseXSO(swagger, function(xso, ncName, isRootXSO, path) {
...
*/
function fixupForNilAndNonNil(swagger, req) {
    ...
    for (let nullableDefName in map.nullableRefs) {
        if (map.nonNullableRefs[nullableDefName]) {
            ...
            if (clonedDef.allOf &&
                ...
                let d = u.getDefNameFromRef(clonedDef.allOf[0].$ref);
...
function c14nXMLObjects(swagger, pure) {
    return u.traverseXSO(swagger, function(xso, nsName, isRootXSO, path) {
        ...
        if (pure) {
            ...
            if (xso.xml) {
                if (isRootXSO || key2 === 'properties'  || key === 'items') {
...
function removeRedundantPrefixes(swagger) {
    return u.traverseXSO(swagger, function(xso, nsName, isRootXSO) {
...
function propogateOccurrence(minItems, maxItems, xibmgroup, xso, swagger, req) {
    if (xso.oneOf || xso.allOf) {
        ...
        for (let i = 0; i < list.length; i++) {
            propogateOccurrence(minItems, maxItems, xibmgroup, list[i], swagger, req);
        }
    }
    if (xso.type == 'array') {
        if (xso.minItems) {
...
function c14nObject(source, c14nList, req) {
    ...
    // Shouldn't have any keys...but move them now just in case
