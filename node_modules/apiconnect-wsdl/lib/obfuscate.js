    ...
    try {
        ...
        if (fileUtils.isZip(inContent)) {
            obfuscateContent(inContent, options).then(function(data) {
                try {
                    fs.writeFileSync(outZip, data.archive.content);
...
function obfuscateContent(inContent, options) {
    ...
    let req = options.req;
    var def = q.defer();
    var isZipDef = q.defer();
    let contentPromise = fileUtils.asContent(inContent, inContent, null, null, req);
    contentPromise.then(function(out) {
        if (fileUtils.isZip(out.content)) {
            isZipDef.resolve(out.content);
        } else {
    ...
    isZipDef.promise.then(function(content) {
        fileUtils.asArchive(content, req).then(function(archive) {
            fileUtils.pipeArchive(archive, req, null, obfuscateFile).then(function(data) {
                def.resolve(data);
            }, function(e) {
                def.reject(e);
            });
...
function obfuscateFile(fileName, fileContent, req) {
    try {
        ...
        // in the output zip.
        let wsdlDoc = d.loadDOM(fileContent);
        if (wsdlDoc) {
            d.removeDTD(wsdlDoc);
            ...
            try {
                let outputDoc = d.loadDOM(fileContent);
