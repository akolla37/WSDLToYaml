...
function merge(files, auth, options) {
    options = options || {};
    var def = q.defer();
    var allWSDLs = [];
    var hasBeenImported = {};
        ...
        if (!alreadySeen[location]) {
            ...
            for (var j = 0; j < len; j++) {
                ...
                if (entry.fullName == location || entry.filename == location) {
                    ...
                    var impDef = null;
                    if (entry.type == 'wsdl') {
                        var tns = entry.json.definitions['undefined'] ? entry.json.definitions['undefined'].targetNamespace : '';
                        if (tns === '') {
...
function processElementNSChildren(children, jsonSchema, req) {
    var len = children.length;
    for (var i = 0; i < len; i++) {
        var child = children[i];
        if (child.xmlns) {
            extendXMLNS(jsonSchema.xmlns, child.xmlns, req);
        }
        if (child.children) {
            processElementNSChildren(child.children, jsonSchema, req);
        }
    } // end for
}

// navigate the DOM to find incidental XMLNS statements
function getElementNS(jsonSchema, elements, req) {
    if (elements && jsonSchema) {
        for (var elemName in elements) {
            var element = elements[elemName];
            if (element.xmlns) {
                extendXMLNS(jsonSchema.xmlns, element.xmlns, req);
            }
            if (element.children) {
                processElementNSChildren(element.children, jsonSchema, req);
            }
        } // end for
    }
}
...
*/
function extendXMLNS(target, source, req) {
    if (source) {
        for (let name in source) {
            ...
            if (target[name] && target[name] !== source[name]) {
                if (name === '__tns__'  && source[name] === 'http://schemas.microsoft.com/2003/10/Serialization/') {
                    ...
                    R.warning(req, 'Detected re-definition of prefix ' + name + ' from ' + target[name] + ' to ' + source[name] +
...
//
function updateSchemaNamespaces(file, req) {
            ...
            for (var i = 0; i < len; i++) {
                ...
                if (jsonSchema['undefined']) {
                    ...
                    if (tns && docSchemas[tns]) {
                        ...
                        getElementNS(jsonSchema, docSchema.elements, req);
                        getElementNS(jsonSchema, docSchema.complexTypes, req);
                        getElementNS(jsonSchema, docSchema.types, req);  // includes simpletypes, attributes, groups, etc.
    ...
    updateWSDLMessageNamespaces(file, req);
}

function updateWSDLMessageNamespaces(file, req) {
                ...
                for (let i = 0; i < len; i++) {
                    ...
                    if (j['undefined']) {
                        ...
                        if (docD.messages && docD.messages[name]) {
                            ...
                            j.xmlns = j.xmlns || {};
                            if (d.xmlns) {
                                extendXMLNS(j.xmlns, d.xmlns, req);
                            }
                            getElementNS(j, d.children, req);
                        }
                    }
                }
            }
