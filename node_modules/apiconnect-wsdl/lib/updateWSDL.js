...
function setServiceEndpoint(filenameOrContent, serviceEndpoints, auth, serviceName, options) {
    ...
    var def = q.defer();
    var fileDef = q.defer();
    var fetchDef = fileUtils.asContent(filenameOrContent, filenameOrContent, auth, null, req);
    fetchDef.then(function(out) {
        var rawContent = out.content;
        var data = [];
        if (fileUtils.isZip(rawContent)) {
            fileUtils.asArchive(rawContent, req).then(function(archive) {
                let outArchivePromise = fileUtils.pipeArchive(archive, req,
                function(fileName) {
                ...
                function(fileName, fileContent, req, originalEncoding) {
                    /**
                    * Process the file
                    * @param fileName
                    ...
                    */
                    try {
                        var wsdlDoc = d.loadDOM(fileContent);
                        ...
                        if (lookForPorts) {
                            ...
                            if (isModified) {
                                ...
                                if (!isUTF8(originalEncoding)) {
                                    str = fileUtils.encode(str, originalEncoding);
                                }
                                data.push({
                                    filename: fileName,
                                    content: str
                ...
                outArchivePromise.then(function(out) {
                    // Don't need the out archive.
                    // But now we can resolve with the updated data list
            ...
            try {
                ...
                var wsdlDoc = d.loadDOM(content.toString());
                var isModified = false;
                ...
                if (isModified) {
                    ...
                    let str = serializer.serializeToString(wsdlDoc);
                    if (!isUTF8(encoding)) {
                        str = fileUtils.encode(str, encoding);
                    }
                    var file = {
                ...
                fileDef.reject(fileUtils.cleanupError(e, req));
            }
        }
